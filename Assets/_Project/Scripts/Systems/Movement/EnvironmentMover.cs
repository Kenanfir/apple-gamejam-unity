...
public class EnvironmentMover : MonoBehaviour
{
    ...
    private void UpdateSpeed()
    {
        ...
        {
            // Get the player's current speed from their rigidbody
            Rigidbody playerRb = playerMotor.GetComponent<Rigidbody>();
            if (playerRb)
            {
                currentSpeed = Mathf.Abs(playerRb.linearVelocity.x);
            }
        }
        else
        {
    ...
    private void MoveEnvironment()
    {
        ...
        // Move all children of the environment parent towards the player
        ...
        foreach (Transform child in environmentParent)
        {
            if (child != null)
            {
                child.Translate(moveDirection, Space.World);
            }
        }
    }
